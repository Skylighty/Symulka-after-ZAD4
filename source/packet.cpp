...
void Packet::GenerateNext() {
  ...
  new_packet->Activate(CurrentTX->CGPgenerator->RandExp(network_->GetLambda()));
}

int Packet::GenerateR() {
  TX *CurrentTX = network_->GetTX(devices_id_);
  return CurrentTX->Rgenerator->Rand(0,((pow(2,r_))-1));
}

int Packet::GenerateCTP(){
  TX *CurrentTX = network_->GetTX(devices_id_);
  return CurrentTX->CTPgenerator->Rand(1,10);
}

int Packet::GenerateT() {
  TX *CurrentTX = network_->GetTX(devices_id_);
  return CurrentTX->Tgenerator->Rand(0,((pow(2,t_))-1));
}

int Packet::GenerateCRP() {
  return GenerateR()*ctp_time_;
}

int Packet::GenerateCP() {
  return GenerateT()*ctp_time_;
}
